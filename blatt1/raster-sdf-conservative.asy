...
void raster_signed_distance(tri tri, bool draw_bb=false, bool use_bb=true, bool conservative=false) {
	...
	for (int y = y_min; y <= y_max; y += 1)
		for (int x = x_min; x <= x_max; x += 1) {
			...
			// Nur für Pixel die erstmal als nicht-getroffen erscheinen, da die folgende Berechnung aufwändig ist.
			else if (conservative) {
				path pixel = (pixel_x-.5,pixel_y-.5)--(pixel_x+.5,pixel_y-.5)--(pixel_x+.5,pixel_y+.5)--(pixel_x-.5,pixel_y+.5)--cycle;
				hit = hit || (intersect(pixel, tri.a--tri.b).length > 0);
...

{
	...
	shipout("raster-tri-cons-1.pdf"); erase();
}
...

{
	...
	shipout("raster-tri-cons-2.pdf"); erase();
}
