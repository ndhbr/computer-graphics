...
{
	// depth buffer
	float depth_buffer[cmdline.vp_w][cmdline.vp_h];
		...
		for (int y = int(bb_min.y); y < int(bb_max.y); ++y)
			for (int x = int(bb_min.x); x < int(bb_max.x); ++x) {
				if (overlaps_pixel(vec2(x+0.5f, y+0.5f), tri, target)) {
					...
					if (depth_buffer[inv_y][x] == 0.0 || depth_buffer[inv_y][x] > tri.a.z) {
						target.framebuffer[inv_y][x] = color_palette[i];
						depth_buffer[inv_y][x] = tri.a.z;
					}
				}
			}
	}
...
double get_z_interpolation(const vec2 &p, const triangle &tri)
{
	return 0.0;
}